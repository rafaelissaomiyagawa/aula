...
public class TestModelFactory {
    ...
    public static final Model<Product> PRODUCT_MODEL
            = Instancio.of(Product.class)
                       .generate(field(Product::getSku), gen -> gen.text()
                                                                   .pattern("#C#C#C-#d#d#d#d#d"))
                       .generate(field(Product::getPrice), gen -> gen.math()
                                                                     .bigDecimal()
                                                                     .min(new BigDecimal("10"))
                                                                     .max(new BigDecimal("100")))
                       .generate(field(Product::getStockQuantity), gen -> gen.ints()
                                                                             .min(1)
                                                                             .max(10))
    ...
    public static final Model<OrderRequest> ORDER_REQUEST_MODEL = Instancio.of(OrderRequest.class)
                                                                           .generate(field(OrderRequest::customerEmail), gen -> gen.net()
                                                                                                                                   .email())
                                                                           .toModel();
    ...
    public static OrderItemRequest createOrderItemRequest(Product product) {
        return Instancio.of(ORDER_ITEM_REQUEST_MODEL)
                        ...
                        .generate(field(OrderItemRequest::quantity), gen -> gen.ints()
                                                                               .range(1, product.getStockQuantity()))
                        .create();
    }
}
