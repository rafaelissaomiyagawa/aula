...
public class TestDatabaseManager {
    ...
    static {
        PGSQL_CONTAINER = new PostgreSQLContainer<>(DOCKER_IMAGE)
                .withDatabaseName(DB_NAME)
                .withUsername(USERNAME)
                .withPassword(PASSWORD)
                .withReuse(true)
                .withCreateContainerCmdModifier(createContainerCmd -> createContainerCmd.withName("test-db"));
    ...
    private static void executePsqlCommand(String command, String successMessage, String errorMessage) {
        try {
            Container.ExecResult execResult = PGSQL_CONTAINER.execInContainer(
                    "psql",
                    "-U", USERNAME,
                    "-d", "postgres",
                    "-c", command
