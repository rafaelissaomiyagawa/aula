...
class InstancioExampleTest {
  ...
  void testGenerateSimpleProduct() {
    Product product =
        Instancio.of(Product.class).set(Select.field(Product::isActive), true).create();
  ...
  void testGenerateListOfOrders() {
    ...
    assertThat(orders).isNotNull().hasSize(3);
    assertThat(orders.getFirst().getOrderNumber()).isNotBlank();
    assertThat(orders.getFirst().getItems()).isNotEmpty();
  }

  @Test
  @DisplayName("should generate an Order ignoring its associated OrderItems")
  void testGenerateOrderIgnoringItems() {
    Order order = Instancio.of(Order.class).ignore(Select.field(Order::getItems)).create();
  ...
  void testGenerateOrderWithSpecificNestedProduct() {
    Order order =
        Instancio.of(Order.class)
            .onComplete(
                Select.field(OrderItem::getProduct),
                (Product product) -> {
    ...
    assertThat(order.getItems().getFirst().getProduct()).isNotNull();
    assertThat(order.getItems().getFirst().getProduct().getName())
        .isEqualTo("Specific Ordered Item");
    assertThat(order.getItems().getFirst().getProduct().getCategory())
        .isEqualTo(ProductCategory.BOOKS);

    assertThat(order.getCustomerEmail()).isNotBlank();
    assertThat(order.getItems().getFirst().getProduct().getPrice()).isGreaterThan(BigDecimal.ZERO);
  }
  ...
  void testGenerateStreamOfProducts() {
    ...
    assertThat(products)
        ...
        .allSatisfy(
            price ->
                assertThat(price).isBetween(new BigDecimal("10.00"), new BigDecimal("100.00")));
  }
  ...
  class UsingTestModelFactory {
    ...
    void shouldOverrideProductValuesUsingTestModelFactory() {
      Product overriddenProduct =
          Instancio.of(TestModelFactory.PRODUCT_MODEL)
